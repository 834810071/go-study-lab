...
type DelayQueue struct {
	...
	readyKey      string // list: 需要投递的消息ID
	unAckKey      string // sorted set: message id -> retry time
	retryKey      string // list: 已到重试时间的ID
	retryCountKey string // hash: message id -> remain retry count
	garbageKey    string // set: message id 用于暂存已达重试上线的消息 ID
	ticker        *time.Ticker
	logger        *log.Logger
	close         chan struct{}

	maxConsumeDuration time.Duration // customizes max consume duration
	msgTTL             time.Duration // TimeToLive
	defaultRetryCount  uint
	fetchInterval      time.Duration // interval at which consumer fetch message
	fetchLimit         uint          // the max number of messages at one time
}
...
// remove unack & msgkey & retryCountKey
